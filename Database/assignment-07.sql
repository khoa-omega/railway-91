...
-- Question 2: Tạo trigger không cho phép người dùng
-- thêm bất kỳ account nào vào department "Sale" nữa,
-- khi thêm thì hiện ra thông báo "Department
...
BEGIN
    ...
    IF v_question_count >= 10 THEN
        SIGNAL SQLSTATE "45000"
        SET MESSAGE_TEXT = "Mỗi exam có tối đa 10 question";
    END IF;
END $$
DELIMITER ;

-- Question 5: Tạo trigger không cho phép
-- người dùng xóa tài khoản có email là admin@gmail.com
...
BEGIN
    IF OLD.email = "admin@gmail.com" THEN
        SIGNAL SQLSTATE "45000"
        SET MESSAGE_TEXT = "Không cho phép xóa tài khoản admin";
    END IF;
END $$
DELIMITER ;

-- Question 6: Tạo trigger cho phép người dùng khi tạo account không điền
-- vào department_id thì sẽ được phân vào "Phòng chờ"
